...
public sealed class ProtoBufExporter
{
    ...
    private void Repeated(StringBuilder file, string newline)
    {
        try
        {
            ...
            file.AppendLine($"\t\tpublic List<{type}> {name} {{ get; set; }} = new List<{type}>();");
        }
        catch (Exception e)
        {
            Log.Error($"{newline}\n {e}");
    ...
    private void Members(StringBuilder file, string currentLine)
    {
        try
        {
            ...
            {
                var summary = currentLine.Split(SplitChars, StringSplitOptions.RemoveEmptyEntries)[^1];
                file.AppendFormat("\t\t///<summary>\r\n" + "\t\t/// {0}\r\n" + "\t\t///</summary>\r\n", $"{summary}。");
            }
            ...
            file.AppendLine($"\t\t[ProtoMember({memberIndex})]");
            if (type == typeCs && FilterType(type) && defaultValue == "null")
            {
                file.AppendLine($"\t\tpublic {typeCs} {name} {{ get; set; }} = new {typeCs}();");
            }
            else
            {
    ...
    private bool FilterType(string type)
    {
        if (type.Contains("[]"))
        {
            return false;
