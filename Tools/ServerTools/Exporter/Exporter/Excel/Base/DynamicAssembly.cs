...
public static class DynamicAssembly
{
    ...
    {
        var configType = dynamicAssembly.GetType($"GameServer.{typeName}");
        
        if (configType == null)
        {
            throw new FileNotFoundException($"GameServer.{typeName} not found");
        }
        
        return configType;
        // return dynamicAssembly.GetType($"GameServer.{typeName}");
    }
