...
namespace GameNetty
{
    ...
    public sealed class MessageDispatcherSystem : Singleton<MessageDispatcherSystem>
    {
        ...
        protected override void OnLoad(int assemblyName)
        {
            ...
            // 如果编译符号GAMESERVER_NET存在，遍历所有实现了IRouteMessageHandler接口的类型，创建实例并添加到_routeMessageHandlers字典中
#if GAMESERVER_NET
        ...
        protected override void OnUnLoad(int assemblyName)
        {
            ...
            // 如果编译符号GAMESERVER_NET存在，移除程序集对应的路由消息处理器信息
#if GAMESERVER_NET
        ...
        // 如果编译符号GAMESERVER_NET存在，定义处理路由消息的方法
#if GAMESERVER_NET
        /// <summary>
