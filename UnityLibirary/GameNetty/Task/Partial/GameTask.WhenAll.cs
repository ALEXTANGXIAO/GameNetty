...
namespace GameNetty
{
    ...
    public partial class FTask<T>
    {
        ...
        /// </summary>
        /// <param name="tasks">要等待的任务列表。</param>
        public static async GameTask WhenAll(List<FTask<T>> tasks)
        {
            if (tasks.Count <= 0)
            {
                return;
            ...
            async GameVoid RunSTask(GameTask tcs, FTask<T> task)
            {
                await task;
                count--;
                if (count == 0)
        ...
        /// </summary>
        /// <param name="tasks">要等待的任务数组。</param>
        public static async GameTask WhenAll(params FTask<T>[] tasks)
        {
            if (tasks == null || tasks.Length <= 0)
            {
                return;
            ...
            async GameVoid RunSTask(FTask<T> task)
            {
                await task;
                count--;
                if (count == 0)
        ...
        /// </summary>
        /// <param name="tasks">要等待的任务数组。</param>
        public static async GameTask WaitAny(params FTask<T>[] tasks)
        {
            if (tasks == null || tasks.Length <= 0)
            {
                return;
